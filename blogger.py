...
class User(db.Model):
    ...
    poste = db.relationship('Post', backref='author', lazy=True)

    #shows how our object will e printed out whenever it prints
    def __repr__(self):
...
def login():
    form = LoginForm()
    if form.validate_on_submit():
        if form.email.data == 'akheirali@gmail.com' and form.password.data == 'password':
